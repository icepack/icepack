...
namespace icepack {
  ...
  namespace basal_shear {
    ...
    nonlinear::nonlinear(const double m, const double tau0, const double u0)
      :
      m(m),
      tau0(tau0),
      u0(u0)
    ...
    double
    nonlinear::operator()(const double beta, const Tensor<1, 2>& u) const
    {
    ...
    linearized::linearized(const double m, const double tau0, const double u0)
      :
      m(m),
      tau0(tau0),
      u0(u0)
    ...
    SymmetricTensor<2, 2>
    linearized::operator()(const double beta, const Tensor<1, 2>& u) const
    {
      const double U = u.norm();
