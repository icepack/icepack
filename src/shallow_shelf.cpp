...
namespace ShallowShelfApproximation
{
  ...
  void ShallowShelf::assemble_system_nonlinear ()
  {
    system_matrix = 0.0;
    system_rhs    = 0.0;
    ...
    std::vector< std::vector< Tensor<1, 2> > >
      velocity_gradients (n_q_points,
                          std::vector< Tensor<1, 2> >(2));
    ...
    for (auto cell: dof_handler.active_cell_iterators())
      {
        ...
        // Getting values of the present solution gradients
        ...
        for (unsigned int q_point = 0; q_point < n_q_points; ++q_point)
          {
            ...
            const SymmetricTensor<2, 2> eps =
              EllipticSystems::get_strain (velocity_gradients[q_point]);
  ...
  } // End of AssembleSystemNonlinear
